...
def strange_game():
    ...
    Суть ее в том, что Вам пк загадано
    любое число от 1 до 100.
    Ваша задача угадть число с помощью подсказок)
    Изначально дается 3 попытки,чтобы "выстерлить"
    наугад,после чего Вам будет доступно еще 3 попытки,но после кадой попытки
